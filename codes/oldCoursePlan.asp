...
<body>
<h3 align="center">请选择需要添加的课程</h3>
<br />
<div align="center">
<form action="oldCoursePlan.asp" method="post" name="form" id="form">
...
<!--分界线-->
<hr color="#9259B0" />
<form action="oldCPinsert.asp" method="post" name="form1" id="form1">
<%
 ...
 rs.open sql,conn,1,3
	response.Write("<table border=1 cellpadding=0 cellspacing=6 align=center class=TableBorder><tbody><tr><td valign=top><table width=100% border=0 align=center><tr><td class=TableName align=center>课程信息表</td></tr></table><table width=100% border=0 align=center>")
	...
	'连接数据库
 sql  ="select oldCourseArrange.cno,cname,TermNum,MajorNum,MaxStu,PresentStu,week,day,hour,wtow,place from oldCourseArrange,CourseInfo where oldCourseArrange.cno=CourseInfo.cno "
 set conn=server.createobject("adodb.connection")
 conn.open "provider=sqloledb;data source=localhost\SQLEXPRESS;uid=sa;pwd=11111;database=web"
 set rs=server.createobject("adodb.recordset")
 rs.open sql,conn,1,3
'显示下学期课程安排
	...
	response.Write("><tbody><tr><td valign=top><table width=100% border=0 align=center><tr><td class=TableName align=center>历史课程安排表</td></tr></table><table width=100% border=0 align=center>")
	response.Write("<td align=center>课程号</td>")
	response.Write("<td align=center>课程名</td>")
	...
	do while not rs.eof
		...
		for i=0 to rs.fields.count-1
			response.Write("<td class=TableFont align=center>"&rs(i)&"</td>")
		next
		response.Write("<td align=center><a href=""oldCPupdate.asp?course_number="&rs(0)&"&term_number="&rs(2)&""">修改</a></td>")
		response.Write("<td align=center><a href=""oldCPdelete.asp?course_number="&rs(0)&"&term_number="&rs(2)&""">删除</a></td>")
		response.Write("</tr>")
		rs.movenext
	loop
end if
