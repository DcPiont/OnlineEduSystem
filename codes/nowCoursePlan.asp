...
<br />
<div align="center">
<form action="nowCoursePlan.asp" method="post" name="form" id="form">
...
<!--分界线-->
<hr color="#9259B0" />
<form action="nowCPinsert.asp" method="post" name="form1" id="form1">
<%
	if (request("button_return")="返回") then
	response.Redirect("nowCoursePlan.asp")
	end if
	'查询课程信息
...
'显示下学期课程安排
	...
	response.Write("><tbody><tr><td valign=top><table width=100% border=0 align=center><tr><td class=TableName align=center>本学期课程安排表</td></tr></table><table width=100% border=0 align=center>")
	response.Write("<td align=center>课程号</td>")
	response.Write("<td align=center>课程名</td>")
	...
	do while not rs.eof
		...
		for i=0 to rs.fields.count-1
			response.Write("<td class=TableFont align=center>"&rs(i)&"</td>")
		next
		response.Write("<td align=center><a href=""nowCPupdate.asp?course_number="&rs(0)&"&term_number="&rs(2)&""">修改</a></td>")
		response.Write("<td align=center><a href=""nowCPdelete.asp?course_number="&rs(0)&"&term_number="&rs(2)&""">删除</a></td>")
		response.Write("</tr>")
		rs.movenext
	loop
end if
