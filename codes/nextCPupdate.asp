...
<title>下学期课程安排信息修改</title>
<!--#include file="connNextCoursePlan.asp"-->
</head>
<%
dim rs
sql="select nextCourseArrange.cno,cname,TermNum,MajorNum,MaxStu,PresentStu,week,day,hour,wtow,place from nextCourseArrange,CourseInfo where nextCourseArrange.cno=CourseInfo.cno and nextCourseArrange.cno='"&request("course_number")&"' and nextCourseArrange.TermNum='"&request("term_number")&"'"
set rs=conn.execute(sql)
%>
<body>
<form name="form1" method="post" action="nextCPupdate.asp">
...
'更新表信息
sql="update nextCourseArrange set MajorNum='"&major_number&"',MaxStu='"&max_student&"',PresentStu='"&present_student&"',week='"&week&"',day='"&jieci&"',hour='"&xueshi&"',wtow='"&wtow&"',place='"&place&"' where cno='"&rs(0)&"' and TermNum='"&rs(2)&"'"
conn.execute sql
conn.close
response.write("<script type=text/javascript>alert('修改成功！');window.location.href='nextCoursePlan.asp';</script>")
end if
%>
</body>
</html>
